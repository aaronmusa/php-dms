$(document).ready(function() {
	...
	$("#list-view").sortable({
		...
		update: function(event, ui) {
			$('label.order').each(function(i, e){
				var value = (i + 1) +'.';
				$(this).html(value);
			});
			var postURL = $("#list-view").data('post-href') ;
			var _token = $('meta[name="csrf-token"]').attr('content');
			var orderList = $("#list-view").sortable('toArray');

			$.each(orderList, function(index, value){
				orderList[index] = value.replace('list-', '');
	...
	$("#edit-article").change(function() {
		...
		$.get(url, {slug: articleSlug}, function(jsonRes) {
			...
			$("#edit-custom-link").val(window.location.hostname + '/articles/' + articleSlug);
			$("#edit-custom-label").val("Read More");
